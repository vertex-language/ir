...
CUDA and HIP device code — the `__global__` and `__device__` half of a `.cu` or `.hip` file — is a second module whose `use` names a GPU. The distance from a CPU module is small and stated in one place: `kernel` as a calling convention, `shared` as a storage domain, §W's work-item verbs and `barrier`, a `scope` on atomics and fences, and the six approximate transcendentals that are one instruction on every GPU. Everything else — arithmetic, memory, control flow, calls — is the same instruction stream, lowered by `ir/lower/ptx` or `ir/lower/amdgpu` instead of a CPU backend. Pointers stay one type: a pointer to workgroup storage is `ptr.getaddr` of a `shared` global, and there is no address space in the type system to get wrong.
