...
// The type layout the device targets give §5's types: how many bytes each
// occupies and what it has to be aligned to. It is the x86-64 table without
// the two extended floats, which is also what nvcc and clang use for device
// code, since a struct has to have the same layout on both sides of a
// kernel launch. The walk itself is the one every backend carries.
...
func scalarSizeAlign(s ir.StoreType) (uint64, uint64, error) {
	...
	case ir.StoreI64, ir.StoreF64, ir.StorePtr:
		return 8, 8, nil
	case ir.StoreF80, ir.StoreF128, ir.StoreV128:
		return 0, 0, fmt.Errorf("%v is not a type the device targets have", s)
	}
